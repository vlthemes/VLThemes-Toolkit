/**
 * VLT Motion Effects (Sticky) Handler
 *
 * Handles sticky positioning for Elementor elements
 */
class VLTStickyHandler extends elementorModules.frontend.handlers.Base {
	currentConfig = {};
	debouncedReactivate = null;

	bindEvents() {
		elementorFrontend.addListenerOnce(this.getUniqueHandlerID() + 'sticky', 'resize', this.reactivateOnResize.bind(this));
	}

	unbindEvents() {
		elementorFrontend.removeListeners(this.getUniqueHandlerID() + 'sticky', 'resize', this.reactivateOnResize.bind(this));
	}
	...
	run(refresh) {
		...
		const activeDevices = this.getElementSettings('sticky_on');
	...
	reactivateOnResize() {
		...
		this.debouncedReactivate = setTimeout(() => {
			...
			const isDifferentConfig = JSON.stringify(config) !== JSON.stringify(this.currentConfig);
...
jQuery(window).on('elementor/frontend/init', () => {
	...
	elementorFrontend.hooks.addAction('frontend/element_ready/container', ($element) => {
		elementorFrontend.elementsHandler.addHandler(VLTStickyHandler, { $element });
	});
